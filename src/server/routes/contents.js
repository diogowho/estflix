...
router.get('/:id', async (req, res) => {
    try {
        const [rows] = await pool.query(`${CONTENT_SELECT} WHERE c.id = ?`, [req.params.id]);
...
router.post('/', async (req, res) => {
    const { title, description, category_id, year, rating, image_url } = req.body;

    if (!title || !category_id || !year || rating === undefined) {
        return res.status(400).json({ message: 'title, category_id, year, and rating are required' });
    }

    try {
        const [result] = await pool.query(
            'INSERT INTO contents (title, description, category_id, year, rating, image_url) VALUES (?, ?, ?, ?, ?, ?)',
            [title, description || null, category_id, year, rating, image_url || null]
        );

        const [rows] = await pool.query(`${CONTENT_SELECT} WHERE c.id = ?`, [result.insertId]);

        return res.status(201).json(rows[0]);
    } catch (err) {
        if (err.code === 'ER_DUP_ENTRY') {
            return res.status(409).json({ message: 'Content title already exists' });
        }
...
router.put('/:id', async (req, res) => {
    try {
        const [existing] = await pool.query(`${CONTENT_SELECT} WHERE c.id = ?`, [req.params.id]);
...
router.delete('/:id', async (req, res) => {
    try {
        const [existing] = await pool.query('SELECT id FROM contents WHERE id = ?', [
            req.params.id,
        ]);
