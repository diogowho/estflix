class AdminPage {
	...
	_buildContentsTable(target) {
		while (target.firstChild) {
			target.removeChild(target.firstChild);
		}

		const contents = this._contentService.getAll();
	...
	_refreshContentsTable() {
		const wrapper = document.getElementById('contents-table-wrapper');
		if (wrapper) this._buildContentsTable(wrapper);
	}

	_showAddContentModal() {
		this._modal.open({
			title: 'ADD CONTENT',
			confirmText: 'Add',
			cancelText: 'Cancel',
			...
			onConfirm: () => {
				...
				try {
					this._contentService.create(data);
					this._modal.close();
		...
		this._appendContentForm(this._modal.getBody(), null);
	}

	_showEditContentModal(content) {
		this._modal.open({
			title: 'EDIT CONTENT',
			confirmText: 'Save',
			cancelText: 'Cancel',
			...
			onConfirm: () => {
				...
				this._modal.setLoading(true);
				try {
					this._contentService.update(content.id, data);
					this._modal.close();
					EstflixToast.show(
		...
		this._appendContentForm(this._modal.getBody(), content);
	}

	_appendContentForm(body, content) {
		const categories = this._categoryService.getAll();
	...
	_showDeleteContentModal(content) {
		this._modal.open({
			...
			onConfirm: () => {
				this._modal.setLoading(true);
				try {
					this._contentService.delete(content.id);
					this._modal.close();
					EstflixToast.show(
	...
	_refreshCategoriesTable() {
		const wrapper = document.getElementById('categories-table-wrapper');
		if (wrapper) this._buildCategoriesTable(wrapper);
	}
	...
	_showEditCategoryModal(category) {
		this._modal.open({
			...
			onConfirm: () => {
				...
				this._modal.setLoading(true);
				try {
					this._categoryService.update(category.id, data);
					this._modal.close();
					EstflixToast.show(
	...
	_appendCategoryForm(body, category) {
		...
		colorInput.value = category?.color ? category.color : '#7b2ff7';

		body.appendChild(this._buildFormGroup('Name', nameInput));
		body.appendChild(this._buildFormGroup('Color', colorInput));
	}
	...
	_refreshProfilesTable() {
		const wrapper = document.getElementById('profiles-table-wrapper');
		if (wrapper) this._buildProfilesTable(wrapper);
	}
	...
	_confirmDeleteProfile(profile) {
		this._modal.open({
			...
			onConfirm: () => {
				this._modal.setLoading(true);
				try {
					this._profileService.delete(profile.id);
					this._modal.close();
					EstflixToast.show(
